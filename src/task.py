...
from losses import margin_loss, exponent_loss
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        for x, y in trainloader:
            ...
            loss, e, s, h = margin_loss(embeddings, y.to(device), alpha)
            # loss, valid_fraction = exponent_loss(embeddings, y.to(device))
            loss.backward()
            optimizer.step()
            total_loss += loss.item()
        print('Epoch {} \t {:.1f}/{:.1f}/{:.1f}  \t Loss: {:.3f}'.format(epoch, e, s, h, total_loss))
